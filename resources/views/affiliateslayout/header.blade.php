     <!DOCTYPE html>
<html lang="en">

  <head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
 <title>@yield('title') - {{config('app.name')}}</title>
    <meta name="description" content=" ">
     <meta name="Author" content=" ">
<meta name="keywords" content=" " />
<meta name="author" content="metatags generator">
<meta name="robots" content="index, follow">
<meta name="revisit-after" content="3 days">
          <meta name="csrf-token" content="{{ csrf_token() }}">
    <link href="{{URL::asset('bussiness/assets/css/bootstrap.min.css')}}" rel="stylesheet">

        <link rel="stylesheet" href="{{URL::asset('bussiness/assets/font-awesome/4.5.0/css/font-awesome.min.css')}}" />

    <!-- page specific plugin styles -->
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/jquery-ui.custom.min.css')}}" />
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/jquery.gritter.min.css')}}" />
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/select2.min.css')}}" />
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/bootstrap-datepicker3.min.css')}}" />
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/bootstrap-editable.min.css')}}" />

    <!-- text fonts -->
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/fonts.googleapis.com.css')}}" />

    <!-- ace styles -->
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/ace.min.css')}}" class="ace-main-stylesheet" id="main-ace-style" />

    <!--[if lte IE 9]>
      <link rel="stylesheet" href="bussiness/assets/css/ace-part2.min.css" class="ace-main-stylesheet" />
    <![endif]-->
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/ace-skins.min.css')}}" />
    <link rel="stylesheet" href="{{URL::asset('bussiness/assets/css/ace-rtl.min.css')}}" />

    <!--[if lte IE 9]>
      <link rel="stylesheet" href="bussiness/assets/css/ace-ie.min.css" />
    <![endif]-->

    <!-- inline styles related to this page -->

    <!-- ace settings handler -->
    <script src="{{URL::asset('bussiness/assets/js/ace-extra.min.js')}}"></script>

  </head>
 
<body class="no-skin">
